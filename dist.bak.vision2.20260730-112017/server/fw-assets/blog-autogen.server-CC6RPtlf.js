...
function slugify(input) {
  return String(input).toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "").replace(/&/g, " and ").replace(/[^a-z0-9\s-]/g, "").trim().replace(/\s+/g, "-").replace(/-+/g, "-").slice(0, 80);
}
async function callAiJson(apiKey, model, system, user) {
...
async function runBlogAutogen(input) {
  ...
  const { data: existing } = await supabaseAdmin.from("blog_posts").select("slug, title").order("updated_at", { ascending: false }).limit(300);
  const existingSlugs = new Set((existing ?? []).map((r) => r.slug));
  const existingTitlesPreview = (existing ?? []).slice(0, 80).map((r) => `- ${r.title}`).join("\n");
  const brainstormSystem = "You are an SEO content strategist for 'Pool Rental Near Me', a peer-to-peer pool rental marketplace. Propose original, search-driven blog topics for pool owners, hosts, and renters. Avoid duplicates of existing posts. Use sentence case for titles.";
  const brainstormUser = `Propose ${count} new blog post ideas${input.topic ? ` in the category "${input.topic}"` : ""}${input.titleHint ? ` related to: "${input.titleHint}"` : ""}.
...
{"ideas":[{"title": string (50-65 chars), "topic": string (short category, 1-3 words), "slug": string (kebab-case, 3-8 words)}]}`;
  ...
  const rawIdeas = Array.isArray(brainstorm?.ideas) ? brainstorm.ideas : [];
  const created = [];
  const errors = [];
  let published = 0;
  for (const raw of rawIdeas.slice(0, count)) {
    ...
    const writeSystem = "You are an expert SEO content writer for 'Pool Rental Near Me'. Write authoritative, useful, original articles in clear American English. Use sentence case for headings. Prefer concrete numbers, steps, and lists. No fluff.";
...
{"seo_title": string (<=60 chars), "seo_description": string (<=160 chars), "excerpt": string (<=200 chars), "tldr_bullets": string[] (3-5 short bullets), "content_markdown": string}`;
    try {
      ...
      const tldr = Array.isArray(article?.tldr_bullets) ? article.tldr_bullets.map((b) => String(b)).slice(0, 5) : null;
      const insertRow = {
        slug,
        title,
        topic,
      ...
      const { error: insErr } = await supabaseAdmin.from("blog_posts").insert(insertRow);
      if (insErr) {
