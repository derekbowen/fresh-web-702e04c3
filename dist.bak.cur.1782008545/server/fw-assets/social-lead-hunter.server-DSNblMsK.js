...
async function runSocialLeadHunt(opts = {}) {
  const sources = opts.sources ?? Object.keys(SOURCE_QUERIES);
  ...
  for (const source of sources) {
    ...
    for (const q of queries) {
      ...
      for (const r of results) {
        ...
        const { data: existing } = await sb().from("social_leads").select("id").eq("source", source).eq("source_url", parsed.source_url).maybeSingle();
        if (existing) {
