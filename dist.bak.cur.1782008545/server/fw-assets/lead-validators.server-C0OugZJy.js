const VALID_AREA_CODES = /* @__PURE__ */ new Set([
  "201",
  "202",
  "203",
  "204",
...
const EMAIL_DOMAIN_STOPLIST = /* @__PURE__ */ new Set([
  "swimply.com",
  "peerspace.com",
  "giggster.com",
  "airbnb.com",
...
const SOURCE_ALLOWLIST = /* @__PURE__ */ new Set([
  "facebook.com",
  "linkedin.com",
  "instagram.com",
  "twitter.com",
...
function normalizePhoneToTen(raw) {
  const digits = raw.replace(/\D/g, "");
  if (digits.length === 11 && digits.startsWith("1")) return digits.slice(1);
  if (digits.length === 10) return digits;
  return null;
}
function validateUSPhone(raw, sourceContext) {
...
function formatPhoneForDisplay(ten) {
...
function isStoplistedUrl(url, opts) {
  try {
    const u = new URL(url);
...
function looksLikeFirstName(name) {
...
function containsBusinessEntity(name) {
  if (!name) return false;
...
function scoreCandidate(input) {
  ...
  const stateMatch = input.host_state && (new RegExp(`\\b${input.host_state.toLowerCase()}\\b`).test(text) || text.includes(`, ${input.host_state.toLowerCase()}`));
