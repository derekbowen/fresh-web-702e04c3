...
export const Route = createFileRoute("/p/pool-pros/$slug")({
  loader: async ({ params }) => {
  ...
  head: ({ loaderData, params }) => {
    ...
    const p = loaderData.provider as any;
    ...
    const category = (p.primary_category as string | null) || "pool-builders";
    ...
    const crumbs = breadcrumbJsonLd([
      ...
      { name: category, path: `/p/pool-pros/c/${category}` },
      ...(stateCode ? [{ name: p.state_code as string, path: `/p/pool-pros/c/${category}/${stateCode}` }] : []),
      ...(stateCode && citySlug && p.city ? [{ name: p.city as string, path: `/p/pool-pros/c/${category}/${stateCode}/${citySlug}` }] : []),
      { name: p.name, path },
    ]);
  ...
  notFoundComponent: () => (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto max-w-3xl px-4 py-20 text-center">
        ...
        <Link to="/p/pool-pros" className="mt-6 inline-flex items-center justify-center rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground">Browse pool pros</Link>
      </main>
      <SiteFooter />
    </div>
  ),
                ...
                <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
                  {(p.city || p.state_code) && (
                    <span>📍 {stateCode && citySlug && p.city ? (
                      <Link to="/p/pool-pros/c/$category/$state/$city" params={{ category, state: stateCode, city: citySlug }} className="hover:text-primary">{p.city}, {p.state_code}</Link>
