...
async function aiGenerate(channel: "email" | "sms" | "dm", lead: LeadCtx, step: number): Promise<{ subject?: string; body: string } | null> {
  ...
  const sys = `You write friendly, ultra-short outreach for Pool Rental Near Me — a Swimply alternative with 0% host fees (vs Swimply's 15%+) — hosts keep 100%. Never mention insurance: PRNM does not provide, include or arrange coverage of any kind. Voice: founder, second person, sentence case, no em dashes, no hype words. Numbers $40-150/hr typical.`;
  const stepHint = step === 1
    ? "First touch. Warm, curious, ask if they'd consider hosting their pool. One soft CTA."
    : step === 2
    ? "Second touch (3 days later). Lead with one concrete benefit (0% host fees or fast payouts). One soft CTA."
