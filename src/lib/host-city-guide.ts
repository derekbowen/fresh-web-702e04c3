...
export function buildHostCityGuide(city: CityRow): HostCityGuide {
  ...
  const sections: HostCityGuideSection[] = [
    ...
    {
      heading: `How PRNM's 0% host fee compares to alternatives`,
      paragraphs: [
        `Pool Rental Near Me charges hosts 0% host fees on every booking — you keep 100%. Swimply takes 15%+ once you stack their host fee, guest fee, and processing. On a $200 booking in ${cityName}, that difference is real money: $0 to PRNM versus $30+ to Swimply, every single time. Across a busy ${state} weekend the gap pays for itself.`,
