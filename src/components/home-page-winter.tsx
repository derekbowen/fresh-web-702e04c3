// useRef is used by HeroSearch below as `useRef<HTMLDivElement>(null)`. It was
// once dropped from this import after `grep -c "useRef("` returned 0 — a pattern
// that cannot match a generic call — and the homepage then shipped throwing
// `ReferenceError: useRef is not defined` on every render. `vite build` does not
// typecheck, so nothing caught it. `npm run check:types` does now.
