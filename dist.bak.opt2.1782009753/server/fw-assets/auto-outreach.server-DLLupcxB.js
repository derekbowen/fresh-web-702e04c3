...
import { t as toE164, i as isOptedOut, s as sendSms } from "./sms.server-BJah3xxU.js";
import "@supabase/supabase-js";
const sb = () => supabaseAdmin;
const AI_URL = "https://ai.gateway.lovable.dev/v1/chat/completions";
const AI_MODEL = "google/gemini-3-flash-preview";
async function loadLead(source, lead_id) {
  if (source === "host_lead") {
...
async function aiGenerate(channel, lead, step) {
  ...
  const stepHint = step === 1 ? "First touch. Warm, curious, ask if they'd consider hosting their pool. One soft CTA." : step === 2 ? "Second touch (3 days later). Lead with one concrete benefit (10% fee or $2M insurance). One soft CTA." : "Last touch (7 days later). Brief, no pressure, leave the door open.";
  const channelHint = channel === "email" ? "Output JSON {subject, body}. Body 70-110 words, plain text with line breaks. Sign 'PRNM team'." : channel === "sms" ? "Output JSON {body}. Max 280 chars. End with 'Reply STOP to opt out.'" : "Output JSON {body}. Max 350 chars, casual DM tone. No links unless natural. No 'STOP' line.";
...
async function enqueueForFollowup(fu) {
  const lead0 = await loadLead(fu.source, fu.lead_id);
  if (!lead0) return { skipped: "no-lead" };
  const lead = { ...lead0, followup_id: fu.id, lead_id: fu.lead_id, source: fu.source };
  let channel;
  let to;
  if (lead.email) {
    channel = "email";
...
async function sendOne(msg, settings) {
...
async function runAutoOutreach() {
  ...
  const { data: newFu } = await sb().from("lead_followups").select("id, source, lead_id").eq("status", "new").order("created_at", { ascending: false }).limit(20);
  let enqueued = 0;
  for (const fu of newFu ?? []) {
    const { count } = await sb().from("auto_outreach_messages").select("id", { count: "exact", head: true }).eq("followup_id", fu.id);
    if ((count ?? 0) > 0) continue;
    try {
      const r = await enqueueForFollowup(fu);
      if (r.inserted) enqueued += r.inserted;
    } catch (e) {
      console.warn("[auto-outreach] enqueue failed", fu.id, e);
    }
  }
  const cap = Math.max(1, Math.floor((settings.max_per_hour ?? 60) / 12));
  const { data: due } = await sb().from("auto_outreach_messages").select("*").eq("status", "pending").lte("scheduled_at", (/* @__PURE__ */ new Date()).toISOString()).order("scheduled_at", { ascending: true }).limit(cap);
  let sent = 0, failed = 0, skipped = 0;
  for (const msg of due ?? []) {
    const r = await sendOne(msg, settings);
    if (r.status === "sent") {
      sent++;
      await sb().from("auto_outreach_messages").update({ status: "sent", sent_at: (/* @__PURE__ */ new Date()).toISOString(), provider_id: r.provider_id ?? null }).eq("id", msg.id);
