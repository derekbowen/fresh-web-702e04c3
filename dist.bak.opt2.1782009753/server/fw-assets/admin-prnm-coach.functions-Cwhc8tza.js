...
function autoDetectRole(email, name) {
...
async function resolveRole(userId) {
  const sbAny = supabaseAdmin;
  const {
    data: existing
  } = await sbAny.from("prnm_coach_user_roles").select("role, source, detected_email, detected_name").eq("user_id", userId).maybeSingle();
  if (existing && (existing.source === "override" || existing.source === "admin_set")) {
    return {
      role: existing.role,
      source: existing.source,
      email: existing.detected_email,
      name: existing.detected_name
    };
  }
  let email = null;
  let name = null;
  try {
    const {
      data: u
    } = await sbAny.auth.admin.getUserById(userId);
  ...
  if (!name) {
    const {
      data: prof
    } = await sbAny.from("profiles").select("display_name, full_name").eq("user_id", userId).maybeSingle();
...
const getCoachRole_createServerFn_handler = createServerRpc({
  id: "1fc6100b81d662bf30f055f485bb8cd24f9120f07ec45923ac6cc1f1c0d45c01",
  name: "getCoachRole",
  filename: "src/server/admin-prnm-coach.functions.ts"
}, (opts) => getCoachRole.__executeServer(opts));
const getCoachRole = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({}).parse(d ?? {})).handler(getCoachRole_createServerFn_handler, async ({
  context
}) => {
  const ctx = context;
...
const setCoachRole_createServerFn_handler = createServerRpc({
  id: "917d54276cd8b450f346cbd32f0f407b0b99ad0b4d05a2bdb61390becda5fcab",
  name: "setCoachRole",
  filename: "src/server/admin-prnm-coach.functions.ts"
}, (opts) => setCoachRole.__executeServer(opts));
const setCoachRole = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
  role: z.enum(["ceo", "coo", "cs"]).nullable()
}).parse(d)).handler(setCoachRole_createServerFn_handler, async ({
  data,
  context
}) => {
  const ctx = context;
  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        days: {
          type: "integer",
          description: "Lookback days (default 14)",
          default: 14
        }
      },
      additionalProperties: false
  ...
  function: {
    ...
    parameters: {
      ...
      properties: {
        issue: {
          type: "string",
          enum: ["thin", "no_meta", "no_keyword", "empty"]
        },
        template_type: {
          type: "string",
          description: "Optional template filter, e.g. host_acq_city"
        },
        limit: {
          type: "integer",
          default: 15
...
async function tool_get_seo_snapshot() {
  const week = new Date(Date.now() - 7 * 864e5).toISOString();
  const [pages, pendingTpl, missing404, gsc, thin, noMeta] = await Promise.all([safe(async () => {
    const [{
      count: total
    }, {
      count: pub
    ...
    const {
      count
    } = await sb.from("content_404_log").select("*", {
      count: "exact",
      head: true
    }).is("resolved_at", null);
    return count ?? 0;
    ...
    const {
      data
    } = await sb.from("gsc_query_data").select("clicks, impressions, captured_at").gte("captured_at", week).limit(5e3);
    const rows = data || [];
    ...
    const {
      data
    } = await sb.from("content_pages").select("body_markdown").eq("status", "published").limit(2e3);
  ...
  return {
    pages,
    pendingByTemplate: pendingTpl,
    unresolved404s: missing404,
    gsc7d: gsc,
    contentQuality: {
      ...thin,
      missingMeta: noMeta
...
async function tool_query_leads(days) {
  const since = new Date(Date.now() - days * 864e5).toISOString();
  const [hostStats, hostRecent, igStats, smsStalled, byCity] = await Promise.all([safe(async () => {
    const [{
      count: total
    }, {
      count: recent
      ...
      count: verified
    }] = await Promise.all([sb.from("host_leads").select("*", {
      count: "exact",
      head: true
    }), sb.from("host_leads").select("*", {
      count: "exact",
      head: true
    }).gte("created_at", since), sb.from("host_leads").select("*", {
      count: "exact",
      head: true
    }).eq("email_sendable", true)]);
    return {
      total: total ?? 0,
      recent: recent ?? 0,
      emailVerified: verified ?? 0
    };
  }, {
    total: 0,
    recent: 0,
    ...
    const {
      data
    } = await sb.from("host_leads").select("name, email, phone_e164, city, region, created_at, page").gte("created_at", since).order("created_at", {
      ascending: false
    }).limit(15);
    return data || [];
  }, []), safe(async () => {
    const [{
      ...
      count: recent
    }] = await Promise.all([sb.from("ig_leads").select("*", {
      count: "exact",
      head: true
    }), sb.from("ig_leads").select("*", {
      count: "exact",
      head: true
    }).eq("contacted", false), sb.from("ig_leads").select("*", {
      count: "exact",
      head: true
    ...
    const {
      count: pending
    } = await sb.from("sms_messages").select("*", {
      count: "exact",
      head: true
    }).eq("status", "pending").lte("scheduled_at", (/* @__PURE__ */ new Date()).toISOString());
    const {
      count: failed
    } = await sb.from("sms_messages").select("*", {
      count: "exact",
    ...
    const {
      data
    } = await sb.from("host_leads").select("city, region").gte("created_at", since).limit(500);
    const counts = {};
    (data || []).forEach((r) => {
  ...
  return {
    lookbackDays: days,
    hostLeads: hostStats,
    recentHostLeads: hostRecent,
    igLeads: igStats,
    smsSequence: smsStalled,
    leadsByCity: byCity
  };
}
async function tool_query_listings() {
  const month = new Date(Date.now() - 30 * 864e5).toISOString();
  const [counts, recent, quality, byState] = await Promise.all([safe(async () => {
    const [{
      count: total
    }, {
      count: pub
    }] = await Promise.all([sb.from("synced_listings").select("*", {
      count: "exact",
      head: true
    }).eq("is_deleted", false), sb.from("synced_listings").select("*", {
      count: "exact",
      head: true
    }).eq("is_deleted", false).eq("state", "published")]);
    return {
      total: total ?? 0,
      published: pub ?? 0
    };
    ...
    const {
      count
    } = await sb.from("synced_listings").select("*", {
      count: "exact",
      head: true
    }).eq("is_deleted", false).gte("st_created_at", month);
    return count ?? 0;
  }, 0), safe(async () => {
    const {
      data
    } = await sb.from("synced_listings").select("title, image_urls, description, capacity, city, state_code, slug").eq("is_deleted", false).eq("state", "published").limit(2e3);
    const rows = data || [];
    const noPhotos = rows.filter((r) => !r.image_urls || r.image_urls.length === 0).length;
    const fewPhotos = rows.filter((r) => r.image_urls && r.image_urls.length > 0 && r.image_urls.length < 4).length;
    const noDesc = rows.filter((r) => !r.description || r.description.length < 100).length;
    const lowCap = rows.filter((r) => r.capacity != null && r.capacity < 5).length;
    const samples = rows.filter((r) => !r.image_urls || r.image_urls.length < 4 || !r.description || r.description.length < 100).slice(0, 8).map((r) => ({
      title: r.title,
      city: r.city,
      state: r.state_code,
      photos: (r.image_urls || []).length,
    ...
    const {
      data
    } = await sb.from("synced_listings").select("state_code").eq("is_deleted", false).eq("state", "published").limit(5e3);
    const counts2 = {};
    (data || []).forEach((r) => {
      const k = r.state_code || "?";
      counts2[k] = (counts2[k] || 0) + 1;
    });
    return Object.entries(counts2).sort((a, b) => b[1] - a[1]).slice(0, 10);
  }, [])]);
  return {
    counts,
    newLast30d: recent,
...
async function tool_query_support(days) {
  const since = new Date(Date.now() - days * 864e5).toISOString();
  const [features, sms404, hot404] = await Promise.all([safe(async () => {
    const {
      data
    } = await sb.from("feature_requests").select("name, email, request_text, city, region, created_at, status").gte("created_at", since).order("created_at", {
      ascending: false
    }).limit(15);
    const {
      count: open
    } = await sb.from("feature_requests").select("*", {
      count: "exact",
      head: true
    }).eq("status", "new");
    return {
    ...
    const {
      data
    } = await sb.from("sms_inbound_log").select("from_phone, body, action, received_at").gte("received_at", since).order("received_at", {
      ascending: false
    }).limit(20);
    const needsReply = (data || []).filter((r) => r.action !== "STOP" && r.action !== "START").length;
    return {
      needsReply,
      recent: data || []
    };
    ...
    const {
      data
    } = await sb.from("content_404_log").select("url_path, hit_count, last_seen_at").is("resolved_at", null).order("hit_count", {
      ascending: false
    }).limit(10);
    return data || [];
  }, [])]);
...
async function tool_query_traffic(days) {
  const since = new Date(Date.now() - days * 864e5).toISOString();
  const [topQueries, topPages, page2] = await Promise.all([safe(async () => {
    const {
      data
    } = await sb.from("gsc_query_data").select("query, clicks, impressions, position").gte("captured_at", since).limit(5e3);
    const agg = {};
    (data || []).forEach((r) => {
      const k = r.query;
      agg[k] = agg[k] || {
    ...
    return Object.entries(agg).map(([q, v]) => ({
      query: q,
      clicks: v.clicks,
      impressions: v.impressions,
      avgPos: +(v.pos / Math.max(v.n, 1)).toFixed(1)
    })).sort((a, b) => b.clicks - a.clicks).slice(0, 15);
  }, []), safe(async () => {
    const {
      data
    } = await sb.from("gsc_query_data").select("url_path, clicks, impressions").gte("captured_at", since).limit(5e3);
    const agg = {};
    (data || []).forEach((r) => {
      agg[r.url_path] = agg[r.url_path] || {
        clicks: 0,
        impressions: 0
      };
    ...
    return Object.entries(agg).map(([p, v]) => ({
      path: p,
      ...v
    })).sort((a, b) => b.clicks - a.clicks).slice(0, 15);
  }, []), safe(async () => {
    const {
      data
    } = await sb.from("gsc_query_data").select("query, url_path, impressions, position, clicks").gte("captured_at", since).gte("position", 11).lte("position", 20).order("impressions", {
      ascending: false
    }).limit(20);
    return data || [];
  }, [])]);
...
async function tool_query_pages(issue, template_type, limit = 15) {
  return safe(async () => {
    ...
    let rows = data || [];
    if (issue === "thin") rows = rows.filter((r) => {
      const w = (r.body_markdown || "").split(/\s+/).filter(Boolean).length;
      return w > 0 && w < 500;
    });
    if (issue === "empty") rows = rows.filter((r) => !(r.body_markdown || "").trim());
    return {
      count: rows.length,
      samples: rows.slice(0, limit).map((r) => ({
        url: r.url_path,
        title: r.title,
...
async function tool_query_revenue_proxies() {
  const [inventory, byState, topPriced, supplyDemand] = await Promise.all([safe(async () => {
    const {
      data
    } = await sb.from("synced_listings").select("price_amount, price_currency").eq("is_deleted", false).eq("state", "published").limit(5e3);
    const rows = (data || []).filter((r) => typeof r.price_amount === "number" && r.price_amount > 0);
    const total = rows.reduce((a, r) => a + (r.price_amount || 0), 0);
    const avg = rows.length ? Math.round(total / rows.length) : 0;
    const sorted = rows.map((r) => r.price_amount).sort((a, b) => a - b);
    const median = sorted.length ? sorted[Math.floor(sorted.length / 2)] : 0;
    return {
      listingsWithPrice: rows.length,
      totalInventoryValue: Math.round(total / 100),
      avgHourlyPrice: Math.round(avg / 100),
      medianHourlyPrice: Math.round(median / 100),
      currency: rows[0]?.price_currency || "USD"
    };
  }, {
    listingsWithPrice: 0,
    ...
    const {
      data
    } = await sb.from("synced_listings").select("state_code, price_amount").eq("is_deleted", false).eq("state", "published").limit(5e3);
    const agg = {};
    (data || []).forEach((r) => {
      if (!r.state_code || !r.price_amount) return;
      agg[r.state_code] = agg[r.state_code] || {
        sum: 0,
        n: 0
      };
      agg[r.state_code].sum += r.price_amount;
      agg[r.state_code].n += 1;
    });
    return Object.entries(agg).map(([s, v]) => ({
      state: s,
      listings: v.n,
      avgHourly: Math.round(v.sum / v.n / 100)
    })).sort((a, b) => b.listings - a.listings).slice(0, 10);
  }, []), safe(async () => {
    const {
      data
    } = await sb.from("synced_listings").select("title, city, state_code, price_amount, slug").eq("is_deleted", false).eq("state", "published").not("price_amount", "is", null).order("price_amount", {
      ascending: false
    }).limit(8);
    return (data || []).map((r) => ({
      title: r.title,
      city: r.city,
      state: r.state_code,
      hourly: Math.round((r.price_amount || 0) / 100),
      slug: r.slug
    }));
  }, []), safe(async () => {
    const [{
      ...
      data: sl
    }] = await Promise.all([sb.from("host_leads").select("city, region").not("city", "is", null).limit(2e3), sb.from("ig_leads").select("query, snippet").limit(1e3), sb.from("synced_listings").select("city, state_code").eq("is_deleted", false).eq("state", "published").limit(5e3)]);
    const supply = {};
    (sl || []).forEach((r) => {
    ...
    const gaps = Object.entries(demand).map(([k, v]) => ({
      city: v.city,
      state: v.state,
      hostLeads: v.n,
      listings: supply[k] || 0
    })).filter((r) => r.hostLeads >= 1 && r.listings <= 1).sort((a, b) => b.hostLeads - a.hostLeads).slice(0, 10);
    return {
      gapCities: gaps,
      totalIgProspects: (il || []).length
    };
  }, {
    gapCities: [],
    totalIgProspects: 0
  ...
  try {
    switch (name) {
      case "get_seo_snapshot":
        return await tool_get_seo_snapshot();
      case "query_leads":
        return await tool_query_leads(Number(args?.days ?? 14));
      case "query_listings":
        return await tool_query_listings();
      case "query_support":
        return await tool_query_support(Number(args?.days ?? 7));
      case "query_traffic":
        return await tool_query_traffic(Number(args?.days ?? 14));
      case "query_revenue_proxies":
        return await tool_query_revenue_proxies();
      case "query_pages":
        return await tool_query_pages(String(args?.issue || "thin"), args?.template_type, Number(args?.limit ?? 15));
      default:
        return {
...
function buildSystemPrompt(role, agentMode = false) {
...
const prnmCoachChat_createServerFn_handler = createServerRpc({
  id: "62ec45a012cbc8ff335d6ca5c87c309b1531c77ea6801cc41c1ad090b461f7e5",
  name: "prnmCoachChat",
  filename: "src/server/admin-prnm-coach.functions.ts"
}, (opts) => prnmCoachChat.__executeServer(opts));
const prnmCoachChat = createServerFn({
...
}).parse(d)).handler(prnmCoachChat_createServerFn_handler, async ({
  data,
  context
}) => {
  const ctx = context;
  ...
  const role = data.roleOverride || resolved.role;
  const completedNote = data.completedRoutes?.length ? `STEPS ALREADY COMPLETED THIS SESSION (do NOT recommend again): ${data.completedRoutes.join(", ")}` : "No steps completed yet this session.";
  const agentMode = !!data.agentMode;
  const messages = [{
    role: "system",
    content: buildSystemPrompt(role, agentMode)
  }, {
    role: "system",
  ...
  for (let iter = 0; iter < MAX_ITERATIONS; iter++) {
    ...
    if (!resp.ok) {
      ...
      return {
        ok: false,
        error: `AI gateway ${resp.status}: ${t.slice(0, 300)}`
      };
    }
    ...
    const results = await Promise.all(toolCalls.map(async (tc) => {
      ...
      return {
        tool_call_id: tc.id,
        role: "tool",
        content: JSON.stringify(result).slice(0, 12e3)
      };
    }));
  ...
  return {
    ok: false,
    error: "Agent exceeded max iterations without final answer"
  };
});
const ALLOWED_ROUTES = /* @__PURE__ */ new Set(["/admin/missing-pages", "/admin/page-auditor", "/admin/keyword-opportunities", "/admin/internal-links", "/admin/seo-health", "/admin/content-pages", "/admin/quick-page", "/admin/generate-content", "/admin/gsc-import", "/admin/competitor-radar", "/admin/rank-tracker", "/admin/indexing", "/admin/link-checker", "/admin/listing-auditor", "/admin/leads", "/admin/ig-lead-hunter", "/admin/contact-enricher", "/admin/feature-requests", "/admin/sms", "/admin/dashboard", "/admin/job-history", "/admin/prnm-coach"]);
const listOpportunities_createServerFn_handler = createServerRpc({
  id: "ebc6e2ce46b7ab45a14db48c398fc0f8f0a6602274228f70b1319517541cd229",
  name: "listOpportunities",
  filename: "src/server/admin-prnm-coach.functions.ts"
}, (opts) => listOpportunities.__executeServer(opts));
const listOpportunities = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
  role: z.enum(["ceo", "coo", "cs"]).optional(),
  includeCompleted: z.boolean().optional()
}).parse(d ?? {})).handler(listOpportunities_createServerFn_handler, async ({
  data,
  context
}) => {
  const ctx = context;
  await assertAdmin(ctx.userId);
  let q = sb.from("prnm_coach_opportunities").select("*").is("dismissed_at", null);
  if (data.role) q = q.eq("role", data.role);
  if (!data.includeCompleted) q = q.is("completed_at", null);
  const {
    data: rows,
    error
  } = await q.order("completed_at", {
    ascending: true,
    nullsFirst: true
  }).order("impact", {
    ascending: true
...
const markOpportunity_createServerFn_handler = createServerRpc({
  id: "95ec2f8872f885682d1152c48fda0b0b1fcd057f88b2e57c05fecddc7dc9b23e",
  name: "markOpportunity",
  filename: "src/server/admin-prnm-coach.functions.ts"
}, (opts) => markOpportunity.__executeServer(opts));
const markOpportunity = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
  id: z.string().uuid(),
  action: z.enum(["complete", "reopen", "dismiss"])
}).parse(d)).handler(markOpportunity_createServerFn_handler, async ({
  data,
  context
}) => {
  const ctx = context;
  ...
  if (data.action === "complete") {
    ...
    patch.dismissed_at = (/* @__PURE__ */ new Date()).toISOString();
  }
  const {
    error
  } = await sb.from("prnm_coach_opportunities").update(patch).eq("id", data.id);
  if (error) return {
    ok: false,
    error: error.message
  };
...
const generateOpportunities_createServerFn_handler = createServerRpc({
  id: "cf812b6a34518ab8a1c67ce80bb408c1d7bd12e83e76ae032f3a50f33b045d29",
  name: "generateOpportunities",
  filename: "src/server/admin-prnm-coach.functions.ts"
}, (opts) => generateOpportunities.__executeServer(opts));
const generateOpportunities = createServerFn({
  method: "POST"
}).middleware([requireSupabaseAuth]).inputValidator((d) => z.object({
  role: z.enum(["ceo", "coo", "cs"]).optional()
}).parse(d ?? {})).handler(generateOpportunities_createServerFn_handler, async ({
  data,
  context
}) => {
  const ctx = context;
  ...
  const resolved = await resolveRole(ctx.userId);
  const role = data.role || resolved.role;
...
${OPP_SCHEMA_PROMPT}`;
  const messages = [{
    ...
    role: "user",
    content: `Generate today's prioritized opportunity list for ${ROLE_LABELS[role]}. Use tools first, then output JSON only.`
  }];
  const toolsUsed = [];
  const MAX_ITER = 10;
  let finalContent = "";
  for (let iter = 0; iter < MAX_ITER; iter++) {
    ...
    if (!resp.ok) return {
      ok: false,
      error: `AI gateway ${resp.status}: ${(await resp.text()).slice(0, 300)}`
    };
    const json = await resp.json();
    const choice = json?.choices?.[0]?.message;
    if (!choice) return {
  ...
  if (!finalContent) return {
    ok: false,
    error: "AI did not return final list"
  };
  let parsed = null;
  ...
  if (opps.length === 0) return {
    ok: false,
    error: "Could not parse opportunities from AI output"
  };
  ...
  const rows = opps.map((o) => {
    ...
    return {
      ...
      why: o.why ? String(o.why).slice(0, 1e3) : null,
  ...
  if (rows.length === 0) return {
    ok: false,
    error: "No valid opportunities to save"
  };
  await sb.from("prnm_coach_opportunities").update({
    dismissed_at: (/* @__PURE__ */ new Date()).toISOString()
  }).eq("role", role).is("completed_at", null).is("dismissed_at", null);
  const {
    error
  } = await sb.from("prnm_coach_opportunities").insert(rows);
  if (error) return {
    ok: false,
    error: error.message
  };
...
export {
  generateOpportunities_createServerFn_handler,
  getCoachRole_createServerFn_handler,
  listOpportunities_createServerFn_handler,
  markOpportunity_createServerFn_handler,
  prnmCoachChat_createServerFn_handler,
  setCoachRole_createServerFn_handler
};
