...
import { c as composeFromPlainText, t as toPlainText, w as wrapInShell, r as renderForRecipient } from "./_shell-Fd6w8MnJ.js";
...
async function getAudienceCount(audience) {
  if (audience === "hosts") {
    const { count } = await supabaseAdmin.from("host_subscribers").select("*", { count: "exact", head: true }).eq("status", "active");
    return count ?? 0;
  }
  if (audience === "renters") {
    const { count } = await supabaseAdmin.from("renter_subscribers").select("*", { count: "exact", head: true }).eq("status", "active");
    return count ?? 0;
  }
  if (audience === "waitlist") {
    const { count } = await supabaseAdmin.from("pool_waitlist").select("*", { count: "exact", head: true });
    return count ?? 0;
  }
  return 0;
}
function unsubSecret() {
  return process.env.UNSUB_TOKEN_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY || "";
}
...
function generateInlineToken(email) {
...
function decodeInlineToken(token) {
  try {
...
async function resolveRecipients(audience, customEmails, singleEmail) {
  if (audience === "single") {
...
async function generateBodyWithAI(opts) {
...
async function sendComposerEmail(opts) {
  ...
  if (campaignId) {
    ...
    const { data: campaign, error: cErr } = await supabaseAdmin.from("composer_campaigns").insert({
  ...
  if (recipients.length === 0) {
    await supabaseAdmin.from("composer_campaigns").update({ status: "completed", completed_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", campaignId);
  ...
  for (const r of recipients) {
    try {
      ...
      const text = plainBody.replaceAll("{{first_name}}", r.firstName || "there") + `
...
Unsubscribe: ${r.unsubscribeUrl}`;
      ...
      failed++;
      if (err?.status === 429) {
        const wait = Math.max((err?.retryAfterSeconds ?? 2) * 1e3, 2e3);
        await new Promise((res) => setTimeout(res, wait));
      }
    }
    await new Promise((res) => setTimeout(res, Math.min(5e3, Math.max(500, opts.delayMs ?? 700))));
  }
  await supabaseAdmin.from("composer_campaigns").update({
    status: "completed",
    sent_count: sent,
...
async function scheduleComposerEmail(opts) {
  if (!opts.subject.trim()) throw new Error("Subject is required");
  ...
  if (when.getTime() < Date.now() - 6e4) throw new Error("Scheduled time is in the past");
...
async function runScheduledComposerEmails() {
  const nowIso = (/* @__PURE__ */ new Date()).toISOString();
  const { data: due } = await supabaseAdmin.from("composer_campaigns").select("id, subject, audience, body_html, plain_body, preview_text, custom_emails, single_email").eq("status", "scheduled").lte("scheduled_at", nowIso).order("scheduled_at", { ascending: true }).limit(5);
  const rows = due || [];
  const results = [];
  for (const row of rows) {
    try {
      const r = await sendComposerEmail({
        ...
        customEmails: row.custom_emails || void 0,
        singleEmail: row.single_email || void 0,
        subject: row.subject,
        bodyHtml: row.body_html,
        bodyText: row.plain_body || void 0,
        preview: row.preview_text || void 0
      });
...
async function unsubscribeComposerByToken(token) {
  const email = decodeInlineToken(token);
  if (!email) return { ok: false };
  const { data: existing } = await supabaseAdmin.from("composer_unsubscribes").select("email").eq("email", email).maybeSingle();
  if (existing) return { ok: true, email, already: true };
  await supabaseAdmin.from("composer_unsubscribes").insert({
    email,
    token: genToken()
  });
